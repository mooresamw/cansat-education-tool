...
import {
  ...
  handleEditMessage,
  getMessages,
  markMessageAsRead, // Import markMessageAsRead
...
export default function StudentMessagePage() {
    ...
    if (selectedInstructor && user) {
      const unsubscribe = getMessages(
        ...
        (newMessages: any[]) => {
          ...
          // Mark all messages from the selected instructor as read
          newMessages.forEach((msg) => {
            if (msg.sender !== user.uid && msg.read?.[user.uid] !== true) {
              markMessageAsRead(
                user.uid,
                [user.uid, selectedInstructor.user_id].sort().join("_"), // Conversation ID
                msg.messageId
              );
            }
          });
  ...
  const handleEditMessageClick = async (messageId: string, newText: string) => {
    ...
    try {
      ...
      setEditingMessageId(null); // Clear editing state after saving
    } catch (error) {
